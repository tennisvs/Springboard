...
SELECT f.name AS facility, 
    ...
    ELSE CONCAT(m.firstname, ' ',  m.surname) END
    AS full_name,
...
SELECT f.name AS facility, 
    ...
    ELSE CONCAT(m.firstname, ' ',  m.surname) END
    FROM Members AS m
    WHERE b.memid = m.memid) AS full_name, 
...
FROM (SELECT f.name AS name, MONTH(TIMESTAMP(starttime)) AS month,
                CASE WHEN b.memid = 0 THEN 0
                ELSE 1 END AS member_used
      FROM Bookings as b 
                LEFT JOIN Facilities AS f
